...
| --- | --- |
| [`kuma-lite/`](./kuma-lite) | Cloudflare Workers + D1 + Cron Triggers で動く Uptime Kuma 風の監視サービス。**本フォークでの追加分**で、上流コードは再利用しないオリジナル実装。詳細は [`kuma-lite/README.md`](./kuma-lite/README.md)。<br>An original (non-derivative) Uptime-Kuma-style monitoring service that runs on Cloudflare Workers + D1 + Cron Triggers. See [`kuma-lite/README.md`](./kuma-lite/README.md). |
